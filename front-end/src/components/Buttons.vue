<template>
    <div v-if="!loading" class="main">
        <div class="members">
            <div class="btn" v-bind:value="user.firstName" v-for="user in this.users" :key="user._id" @click="selectUser(user, $event)">{{user.firstName}}</div>
        </div>
...
export default {
    ...
    methods: {
      ...
      this.$root.$data.user = null;
        ...
        async clockIn() {
            if(this.activeUser.isClockedIn === true)
            return alert("Already clocked in");
      try {
        await axios.post("/api/time", {
            activeUser: this.activeUser,
            clockIn: moment().format()
        });
        this.activeUser.isClockedIn = true;
        this.getClockedIn();
        return true;
      } catch (error) {
        //console.log(error);
    ...
    async ClockOut(){
        if(this.activeUser.isClockedIn === false)
        return alert("Select user and clock in first!");
        try {
        await axios.put('/api/time/' + this.activeUser._id, {
            clockOut: moment().format()
        });
        this.activeUser.isClockedIn = false;
        this.getClockedIn();
